...
per-client :class:`~robot_friend.diagnostics.panels.log_panel.LogPanel` polls
...
class LogSource(DashboardDataSource):
    ...
    def start(self, bus: Bus) -> None:
        ...
        finch_logger.info("diagnostics logging initialised")
...
_NOMINAL_LOGS = [
    "12:00:01 INFO    robot_friend.diagnostics: dashboard started (demo mode)",
    ...
    "12:00:03 WARNING robot_friend.speech.audio: input overrun, dropped 1 block",
    "12:00:03 INFO    robot_friend.speech: transcript 'hey finch' (de, conf=0.88)",
    "12:00:04 ERROR   robot_friend.serial: no device on /dev/ttyACM0 (will retry)",
]

_STRESS_LOGS = [
    "12:00:01 INFO    robot_friend.diagnostics: " + "a very long log line meant to test horizontal overflow and wrapping behaviour " * 4,
    "12:00:02 DEBUG   robot_friend.image.backends.ultralytics.YOLODetector: boxes=["
    + ", ".join(f"(x1={i*37},y1={i*11},x2={i*37+120},y2={i*11+240},conf=0.9{i})" for i in range(8)) + "]",
    "12:00:03 WARNING robot_friend.speech.audio.vad_segmenter: rms=0.0123456789 thr=0.02 — segment too short ✋🔊 (unicode: äöü ß 你好 こんにちは 🤖)",
