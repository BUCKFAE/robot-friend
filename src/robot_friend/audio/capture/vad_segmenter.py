...
class VadSegmenter:
    ...
    def push(self, block: np.ndarray) -> np.ndarray | None:
        """Feed one fixed-size audio block.

        Args:
            block: One block of mono float32 samples.

        Returns:
            The finished utterance once trailing silence (or the max length)
            closes it out, otherwise None while the utterance is still open.
        """
    ...
    def flush(self) -> np.ndarray | None:
        """Close the current utterance and reset.

        Returns:
            The buffered utterance if it met the minimum speech duration,
            otherwise None.
        """
