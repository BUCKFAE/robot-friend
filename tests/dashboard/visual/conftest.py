"""Fixtures for the dashboard's Playwright visual + E2E suite.
...
run never imports the browser fixtures. Run the suite with ``just setup`` then
...
def _wait_ready(proc: subprocess.Popen, url: str, timeout: float = 40.0) -> None:
    ...
    while time.monotonic() < end:
        if proc.poll() is not None:
            ...
            raise RuntimeError(f"dashboard server exited early (rc={proc.returncode}):\n{out}")
        try:
    ...
    proc.terminate()
    out = proc.stdout.read().decode(errors="replace") if proc.stdout else ""
    raise RuntimeError(f"dashboard server not ready at {url} within {timeout}s:\n{out}")


@contextlib.contextmanager
def _server(scenario: str, port: int):
    """Run the dashboard with fake data on ``port`` for the duration of the block."""
    env = {**os.environ, "DASHBOARD_HOST": HOST, "DASHBOARD_PORT": str(port)}
